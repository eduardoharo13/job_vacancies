...
GROUP_B_COLS = ['esco_skill', 'esco_skill_broader_hierarchy_fn','esco_broader_skill_fn',
...
GROUP_C_COLS = ['isco_final_label', 'has_isco_0', 'has_isco_1', 
                'has_isco_2', 'has_isco_3', 'has_isco_4', 'has_isco_5', 'has_isco_6', 
                'has_isco_7', 'has_isco_8', 'has_isco_9', 'has_Low_skill', 'has_Medium_skill', 
                'has_High_skill']
GROUP_D_COLS = ['at_least_one_digital_skill','esco_digital_skill', 'at_least_one_green_skill','esco_green_skill'] 
...
def format_frequency_df(df_frequency, total_series):
    ...
    total_count_val = total_series.iloc[0]
    
    # Check if the DataFrame has any data to prevent errors if empty
    if df_frequency.empty:
        total_percentage = 0.0
    else:
        # Calculate the total percentage (which can be > 100% for multi-label columns)
        total_percentage_calculated = df_frequency['Percentage (%)'].sum().round(2)
        
        # --- MODIFICACIÓN CLAVE (Ventana de Tolerancia Ampliada) ---
        
        # 1. Caso de Multi-Label basado en filas (puede exceder 100%, como 126.66% para 'region').
        # Si excede un umbral muy cercano a 100, forzamos 100.00% porque el denominador es la población base (filas).
        if total_percentage_calculated > 100.05:
            total_percentage = 100.00
            
        # 2. Caso de Error de Redondeo (99.75%, 99.98%, o 100.02%).
        # Si el valor está entre 99.50% y 100.05%, es casi seguro un error de redondeo y forzamos 100.00%.
        elif 99.50 <= total_percentage_calculated <= 100.05:
            total_percentage = 100.00
            
        # 3. Otros casos (generalmente irrelevantes en este contexto)
        else:
            total_percentage = total_percentage_calculated
    
    # Create the Total row
    total_row = pd.Series({
        'Count': int(total_count_val), 
...
# --- PERCENTAGE CALCULATION SELECTOR (For non-rigid multi-label columns) ---
    if selected_col in MULTI_LABEL_COLUMNS and selected_col not in COUNT_BY_ROW_COLUMNS:
        ...
        pct_options = ["Total count of label occurrences", "Total count of non-null observations"]
        
        # MODIFICACIÓN: Agregar opción específica para Digital/Green skills
        if selected_col in ['esco_digital_skill', 'esco_green_skill','esco_broader_skill_fn']:
            pct_options.append("Total count of non-null observations (esco_skill)")
        ...
        percentage_mode = st.sidebar.radio(
            ...
            index=0,
            key='percentage_mode',
            help="Defines the denominator for percentage calculation."
        )
    else:
        # Use a temporary state variable to avoid reference errors
        percentage_mode = None 
        if selected_col in COUNT_BY_ROW_COLS and selected_col is not None:
...
# --- Denominator Assignment based on column and selector ---
        ...
        # MODIFICACIÓN: Lógica para el denominador basado en 'esco_skill'
        elif percentage_mode == "Total count of non-null observations (esco_skill)":
            if 'esco_skill' in df_analysis.columns:
                # Calcular filas donde esco_skill no es vacío (len > 0)
                esco_skill_count = (df_analysis['esco_skill'].apply(lambda x: len(x) > 0 if x is not None else False)).sum()
                denominator = esco_skill_count
                total_for_format = pd.Series([denominator], index=['TOTAL'])
                st.caption(f"Note: Percentage is calculated based on total NON-EMPTY rows of 'esco_skill' ({denominator}).")
            else:
                st.error("Variable 'esco_skill' not found in dataset.")
                denominator = 1 # Evitar división por cero
                total_for_format = pd.Series([denominator], index=['TOTAL'])
        ...
        else: # Default: Label occurrences
...
# New Percentage Mode Selector for Cross-Tabulation
    if cross_col and selected_col in MULTI_LABEL_COLUMNS:
        st.markdown("##### Percentage Configuration")
        
        # Definir opciones base para crosstab
        ct_options = ["Total count of label occurrences", "Total count of non-null observations (Rows)"]
        
        # MODIFICACIÓN: Agregar opción específica
        if selected_col in ['esco_digital_skill', 'esco_green_skill','esco_broader_skill_fn']:
            ct_options.append("Total count of non-null observations (esco_skill)")
            
        cross_tab_percentage_mode = st.radio(
            "Percentage Base (%) for Grouped Analysis",
            options=ct_options,
            index=0,
            key='cross_tab_percentage_mode',
            help="Defines the denominator logic for the grouped analysis."
        )
    else:
    ...
    if cross_col:
        ...
        if selected_col in MULTI_LABEL_COLUMNS:
            ...
            # NOTE: We keep this label to prevent conflict with a real label called 'TOTAL'
            ...
            if cross_tab_percentage_mode == "Total count of non-null observations (Rows)":
                ...
                caption_mode = "total non-null unique rows/observations"

            # MODIFICACIÓN: Nueva lógica para esco_skill en grupos
            elif cross_tab_percentage_mode == "Total count of non-null observations (esco_skill)":
                if 'esco_skill' in df_analysis.columns:
                    # Reutilizamos la función calculate_row_denominators pero pasándole 'esco_skill' como columna principal
                    # Esto contará cuántas filas tienen 'esco_skill' lleno por cada grupo de 'cross_col'
                    df_esco_denominators = calculate_row_denominators(df_analysis, cross_col, 'esco_skill')
                    group_totals_series = df_esco_denominators['Total_Rows']
                    
                    df_results_with_denominators = df_download_results.join(df_esco_denominators, on=cross_col)
                    
                    # Cálculo del porcentaje
                    df_download_results['Percentage (%)'] = (
                        df_results_with_denominators['Count'] / df_results_with_denominators['Total_Rows'] * 100
                    ).round(2)
                    caption_mode = "total non-null observations of 'esco_skill'"
                else:
                    st.error("Column 'esco_skill' not found for denominator calculation.")
                    group_totals_series = None
                ...
                caption_mode = "total label occurrences (sum of all mentioned labels) "

            
            # --- 3. ADD A SINGLE TOTAL ROW PER GROUP (NO SORTING HERE YET) ---
            total_rows_list = []
            
            if group_totals_series is not None:
                for group_label, total_count in group_totals_series.items():
                    # Use the temporary label
            ...
            if total_rows_list:
                df_download_results = pd.concat([df_download_results] + total_rows_list)
            ...
            # First, sort data rows by Count descendingly within each group
            # --- CORRECCIÓN DEL ERROR DE TYPEERROR (ELIMINACIÓN DE 'level') ---
            df_data_sorted = df_data.sort_values(by=['Count', df_data.index.names[1]], 
                                                 ascending=[False, True]) 
            # Apply Top N filtering
            df_top_n = df_data_sorted.groupby(level=0, group_keys=False).head(top_n)
            
            # Combine Top N data rows with all Total rows for final display
            df_display_results_final = pd.concat([df_top_n, df_totals])
            
            # **MODIFICACIÓN CLAVE PARA EL ORDENAMIENTO DE LA VISUALIZACIÓN**
            # Ordenar primero por el grupo (level 0) alfabéticamente.
            # Luego crear una columna temporal para forzar que 'TOTAL' (temp_total_label) vaya al final.
            df_display_fixed_temp = df_display_results_final.reset_index()
            df_display_fixed_temp['sort_key'] = df_display_fixed_temp[selected_col].apply(
                lambda x: 1 if x == temp_total_label else 0
            )
            df_display_fixed = df_display_fixed_temp.sort_values(
                by=[cross_col, 'sort_key', 'Count'],
                ascending=[True, True, False]
            ).drop(columns=['sort_key']).set_index([cross_col, selected_col])
            ...
            df_display_fixed = df_display_fixed.reset_index()
            ...
            with col_table:
                ...
                st.caption(f"Filtered Table: Showing Top {top_n} results per group (includes 'TOTAL' row per group).")
                st.caption(f"Percentage is calculated based on the **{caption_mode}** WITHIN each group ('{cross_col.capitalize()}').")

            with col_download:
                
                # **LÓGICA PARA ORDENAMIENTO DE EXCEL (TOTAL AL FINAL)**
                # 1. Crear una copia y resetear el índice para operar
                df_export_results = df_download_results.copy().reset_index()
                
                # 2. Crear una columna de ordenamiento temporal: 1 para el total, 0 para los datos.
                df_export_results['sort_key'] = df_export_results[selected_col].apply(
                    lambda x: 1 if x == temp_total_label else 0
                )
                
                # 3. Ordenar: 
                #    - Por el grupo (cross_col) alfabéticamente (True).
                #    - Por la clave de ordenamiento (sort_key) (True, 0 antes de 1).
                #    - Por el conteo (Count) descendente (False) para mantener el Top N original dentro de los datos.
                df_export_results = df_export_results.sort_values(
                    by=[cross_col, 'sort_key', 'Count'],
                    ascending=[True, True, False]
                )
                
                # 4. Eliminar la columna temporal y renombrar el label 'TOTAL'
                df_export_results = df_export_results.drop(columns=['sort_key'])
                df_export_results[selected_col] = df_export_results[selected_col].replace(temp_total_label, 'TOTAL')
                
                # 5. Volver a establecer el MultiIndex antes de exportar
                df_export_results = df_export_results.set_index([cross_col, selected_col])
